...
def lookback_time(cascades, num_users, lookback_count):
    usermatrix = np.zeros((num_users, num_users))
    for cascade in cascades:
        for pos in range(len(cascade)):
            i, ti = cascade[pos]
            for prev_pos in range(0, pos):
                j, tj = cascade[prev_pos]
                if ti - tj < lookback_count:
                    usermatrix[i][j] = 1
    return usermatrix       
...
def _create_base_graph(train_cascades, idx2u, lookback_count=None, edge_thr=5, usertimematrix=None):
    """
    Returns
    -------
    base_graph : PNEANet
...
def load_data_for_parameter_estimation(cascades_filename, labels_filename,
    ...
    if isinstance(lookback_count, float):
        usertimematrix = lookback_time(indexed_cascades, len(u2idx), lookback_count)
    else:
        usertimematrix = None
    base_graph = _create_base_graph(train_cascades, idx2u, lookback_count, edge_thr, usertimematrix)
    return u2idx, idx2u, train_cascades, train_labels, \
        filtered_train_cids, test_cascades, test_labels, base_graph, usertimematrix
